...
// MARK: - Karnataka College Model
struct KarnatakaCollege: Identifiable, Decodable {
    let id: String
    let name: String
    let address: String
    let state: String
    ...
    // Flexible decoder: accepts id as Int or String, safe defaults
...
// MARK: - Karnataka API Response
struct KarnatakaAPIResponse: Decodable {
    let success: Bool
    let colleges: [KarnatakaCollege]?
    let message: String?
}

// MARK: - Karnataka View
struct Karnataka: View {
    @Environment(\.presentationMode) var presentationMode
    @State private var colleges: [KarnatakaCollege] = []
    ...
    @ViewBuilder
    func destinationView(for college: KarnatakaCollege) -> some View {
        let name = college.name.lowercased()

        if name.contains("national institute of technology") {
            NITKView()
        } else if name.contains("visvesvaraya") || name.contains("vtu") {
            VTUView()
        } else {
            VStack(spacing: 12) {
                Text("College Details").font(.headline)
                Text(college.name).font(.title)
                Text(college.address).font(.subheadline)
                Text(college.state).font(.caption).foregroundColor(.gray)
            }
            .padding()
            .navigationBarBackButtonHidden(true)
        }
    ...
    // MARK: - Fetch from API
    func fetchColleges() {
        guard let url = URL(string: ServiceAPI.kaURL) else {
        ...
        URLSession.shared.dataTask(with: url) { data, _, error in
            ...
            do {
                let apiResponse = try decoder.decode(KarnatakaAPIResponse.self, from: data)
                DispatchQueue.main.async {
                ...
                do {
                    let fallback = try decoder.decode([KarnatakaCollege].self, from: data)
                    DispatchQueue.main.async {
